...
The zoo supports `count` (an aggregator that counts the number of events seen), `mean` (an aggregator that computes the arithmetic mean of a numeric value extracted from each event by a caller-supplied function), and `zip_all` (a combinator that runs a list of aggregators in parallel over the same event stream and returns a list of their results). `zip_all` is the variadic generalisation of the existing binary `zip_agg`.
